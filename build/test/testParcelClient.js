...
const parcel_client_1 = require("../src/lib/parcel-client");
/**
 ...
 */
function createTestClient(apiKey, port) {
    const client = new parcel_client_1.ParcelClient(apiKey);
    // Override the private request method to use our local HTTP server
    client["request"] = function (method, path, authenticated, body) {
    ...
    describe("getDeliveries", () => {
        it("should return deliveries on success", async () => {
            ...
            try {
                ...
                (0, chai_1.expect)(result).to.deep.equal(deliveries);
            }
            finally {
        ...
        it("should throw on API error with INVALID_API_KEY code", async () => {
            ...
            try {
                ...
                const error = err;
                (0, chai_1.expect)(error.code).to.equal("INVALID_API_KEY");
        ...
        it("should throw on API error with generic error code", async () => {
            ...
            try {
                ...
                const error = err;
                (0, chai_1.expect)(error.code).to.equal("API_ERROR");
                (0, chai_1.expect)(error.message).to.include("Something went wrong");
            }
            finally {
                await stopServer(server);
            }
        ...
        it("should send api-key header", async () => {
            ...
            const { server, port } = await startMockServer((req, res) => {
                receivedApiKey = req.headers["api-key"];
                res.writeHead(200, { "Content-Type": "application/json" });
                res.end(JSON.stringify({ success: true, deliveries: [] }));
            });
            try {
                ...
                await client.getDeliveries("active");
                (0, chai_1.expect)(receivedApiKey).to.equal("my-secret-key");
            }
            finally {
                await stopServer(server);
            }
        ...
        it("should pass filter_mode in query string", async () => {
            ...
            try {
                ...
                (0, chai_1.expect)(receivedPath).to.include("filter_mode=recent");
            }
            finally {
                await stopServer(server);
            }
        ...
        it("should default to active filter", async () => {
            ...
            try {
                ...
                (0, chai_1.expect)(receivedPath).to.include("filter_mode=active");
            }
            finally {
                await stopServer(server);
            }
    ...
    describe("HTTP error handling", () => {
        it("should detect rate limiting (429)", async () => {
            ...
            try {
                ...
                const error = err;
                (0, chai_1.expect)(error.code).to.equal("RATE_LIMITED");
                (0, chai_1.expect)(error.retryAfterSeconds).to.equal(120);
                (0, chai_1.expect)(error.message).to.include("Rate limit");
            }
            finally {
                await stopServer(server);
            }
        ...
        it("should use default retry-after when header missing", async () => {
            ...
            try {
                ...
                const error = err;
                (0, chai_1.expect)(error.code).to.equal("RATE_LIMITED");
                (0, chai_1.expect)(error.retryAfterSeconds).to.equal(300); // 5 * 60
            }
            finally {
                await stopServer(server);
            }
        ...
        it("should return HTTP_ERROR for 500", async () => {
            ...
            try {
                ...
                const error = err;
                (0, chai_1.expect)(error.code).to.equal("HTTP_ERROR");
                (0, chai_1.expect)(error.message).to.include("500");
            }
            finally {
                await stopServer(server);
            }
        ...
        it("should throw on invalid JSON response", async () => {
            ...
            try {
                ...
                const error = err;
                (0, chai_1.expect)(error.message).to.include("JSON parse error");
            }
            finally {
                await stopServer(server);
            }
    ...
    describe("getCarrierNames", () => {
        it("should return carrier map", async () => {
            ...
            try {
                ...
                (0, chai_1.expect)(result).to.deep.equal(carriers);
            }
            finally {
                await stopServer(server);
            }
        ...
        it("should return empty map on error without caching", async () => {
            ...
            try {
                ...
                (0, chai_1.expect)(result2).to.deep.equal({ dhl: "DHL" });
        ...
        it("should not send api-key header for carrier names", async () => {
            ...
            try {
                ...
                await client.getCarrierNames();
                (0, chai_1.expect)(receivedApiKey).to.be.undefined;
            }
            finally {
                await stopServer(server);
            }
    ...
    describe("getCarrierName", () => {
        it("should resolve carrier code to name", async () => {
            ...
            try {
                ...
                (0, chai_1.expect)(name).to.equal("DHL Express");
            }
            finally {
                await stopServer(server);
            }
        ...
        it("should return uppercase code when not found in map", async () => {
            ...
            try {
                ...
                (0, chai_1.expect)(name).to.equal("UNKNOWN_CARRIER");
            }
            finally {
                await stopServer(server);
            }
    ...
    describe("addDelivery", () => {
        it("should POST delivery data", async () => {
            ...
            try {
                ...
                (0, chai_1.expect)(receivedMethod).to.equal("POST");
                (0, chai_1.expect)(result.success).to.be.true;
                const body = JSON.parse(receivedBody);
                (0, chai_1.expect)(body.tracking_number).to.equal("123");
                (0, chai_1.expect)(body.carrier_code).to.equal("dhl");
                (0, chai_1.expect)(body.description).to.equal("Test");
            }
            finally {
                await stopServer(server);
            }
        ...
        it("should return error response", async () => {
            ...
            try {
                ...
                (0, chai_1.expect)(result.error_message).to.equal("Duplicate");
            }
            finally {
                await stopServer(server);
            }
    ...
    describe("API-drift guards", () => {
        ...
        it("should throw API_ERROR when response is null", async () => {
            ...
            try {
                ...
                const error = err;
                (0, chai_1.expect)(error.code).to.equal("API_ERROR");
                (0, chai_1.expect)(error.message).to.include("malformed");
            }
            finally {
                await stopServer(server);
            }
        ...
        it("should handle non-string error_code/error_message (API drift)", async () => {
            ...
            try {
                ...
                const error = err;
                (0, chai_1.expect)(error.code).to.equal("API_ERROR");
                (0, chai_1.expect)(error.message).to.include("UNKNOWN");
            }
            finally {
                await stopServer(server);
            }
        ...
        it("getCarrierName should return UNKNOWN for non-string input", async () => {
            ...
            try {
                ...
                (0, chai_1.expect)(name1).to.equal("UNKNOWN");
                (0, chai_1.expect)(name2).to.equal("UNKNOWN");
                (0, chai_1.expect)(name3).to.equal("UNKNOWN");
                (0, chai_1.expect)(name4).to.equal("UNKNOWN");
            }
            finally {
                await stopServer(server);
            }
        ...
        it("getCarrierName should fall back to uppercase code for non-string map value", async () => {
            ...
            try {
                ...
                (0, chai_1.expect)(dhl).to.equal("DHL");
                (0, chai_1.expect)(ups).to.equal("UPS");
                (0, chai_1.expect)(fedex).to.equal("FedEx");
            }
            finally {
                await stopServer(server);
            }
    ...
    describe("testConnection", () => {
        it("should return success when API responds", async () => {
            ...
            try {
                ...
                (0, chai_1.expect)(result.success).to.be.true;
                (0, chai_1.expect)(result.message).to.equal("Connection successful");
            }
            finally {
                await stopServer(server);
            }
        ...
        it("should return failure for invalid API key", async () => {
            ...
            try {
                ...
                const result = await client.testConnection();
                (0, chai_1.expect)(result.success).to.be.false;
                (0, chai_1.expect)(result.message).to.equal("Invalid API key");
            }
            finally {
                await stopServer(server);
            }
        ...
        it("should return failure with error message for other errors", async () => {
            ...
            try {
                ...
                const result = await client.testConnection();
                (0, chai_1.expect)(result.success).to.be.false;
                (0, chai_1.expect)(result.message).to.include("500");
            }
            finally {
                await stopServer(server);
            }
